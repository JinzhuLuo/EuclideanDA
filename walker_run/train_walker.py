...
class Workspace:
    ...
    def get_eq_state(self, env):
        ...
        pos_torso = env.physics.named.data.xpos['torso'][None, :].flatten()
        pos_right_thigh = env.physics.named.data.xpos['right_thigh'][None, :].flatten()
        pos_right_leg = env.physics.named.data.xpos['right_leg'][None, :].flatten()
        pos_right_foot = env.physics.named.data.xpos['right_foot'][None, :].flatten()

        pos_left_thigh = env.physics.named.data.xpos['left_thigh'][None, :].flatten()
        pos_left_leg = env.physics.named.data.xpos['left_leg'][None, :].flatten()
        pos_left_foot = env.physics.named.data.xpos['left_foot'][None, :].flatten()
        ...
        vel_right_leg = env.physics.data.object_velocity("right_leg", "geom", local_frame=False)[0:1, :].flatten()
        vel_right_foot = env.physics.data.object_velocity("right_foot", "geom", local_frame=False)[0:1, :].flatten()
        vel_left_thigh = env.physics.data.object_velocity("left_thigh", "geom", local_frame=False)[0:1, :].flatten()
        vel_left_leg = env.physics.data.object_velocity("left_leg", "geom", local_frame=False)[0:1, :].flatten()
        vel_left_foot = env.physics.data.object_velocity("left_foot", "geom", local_frame=False)[0:1, :].flatten()
        pos_direction = env.physics.named.data.xmat[1:, ['xx', 'xy', 'xz']].flatten()

        eq_state = np.concatenate((pos_direction, vel_torso, vel_right_thigh, vel_right_leg, vel_right_foot,
                                   vel_left_thigh, vel_left_leg, vel_left_foot), axis=0)

        eq_state = eq_state.reshape(-1)
        hight = env.physics.torso_height().flatten()
        eq_state = np.concatenate((eq_state, hight), axis=0)
        eq_state = eq_state.reshape(-1)
        # print(eq_state)
...
def main(cfg):
    from train_walker import Workspace as W
    root_dir = Path.cwd()
    workspace = W(cfg)
