...
#        next_obs = next_obs.float()
        ...
        print(num_batch, dim_input)
        noise = np.random.normal(0, std, num_batch*dim_input).reshape(num_batch, -1)
        mask = np.random.uniform(0, 1, num_batch).reshape(num_batch, -1) < prob
        noise = torch.tensor(noise * mask).to(self.device)
