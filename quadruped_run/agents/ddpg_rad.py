...
#        next_obs = next_obs.float()
        ...
        lower = 0.5
        upper = 1.5
        random_number_1 = np.random.uniform(lower, upper, obs.shape[0]).reshape(-1,1)
        random_number_2 = np.random.uniform(lower, upper, obs.shape[0]).reshape(-1,1)
        
        obs = obs * torch.tensor(random_number_1).to(self.device)
        next_obs = next_obs * torch.tensor(random_number_1).to(self.device)
