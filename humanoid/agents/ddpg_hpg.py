...
class DDPGAgent:
    def __init__(self, obs_shape, action_shape, device, lr, feature_dim,
        ...
        self.abstract_critic = DDPGCritic(obs_shape, action_shape[0],
                                          hidden_dim, linear_approx).to(self.device)
    ...
    def update_abstract_critic(self, obs, action, reward, discount, next_obs, step):
        ...
        obs, action, next_obs = self.get_aug_data(torch.clone(obs), torch.clone(action),
                                                            torch.clone(next_obs))
        with torch.no_grad():
            if self.clipped_noise:
                # Select action according to policy and add clipped noise
                stddev = utils.schedule(self.stddev_schedule, step)
    ...
    def update_abstract_actor(self, obs, step):
        metrics = dict()

        # Compute actor loss
        actor_loss = -self.abstract_critic(obs, self.actor(obs)).mean()
    ...
    def update_actor(self, eq_state, step):
        ...
        # Compute actor loss
        actor_loss = -self.critic(eq_state, self.actor(eq_state)).mean()

        abstract_actor_loss, metrics_update = self.update_abstract_actor(eq_state, step)
    ...
    def update(self, replay_iter, step):
        ...
        metrics.update(self.update_abstract_critic(eq_state, action, reward, discount, next_eq_state, step))
