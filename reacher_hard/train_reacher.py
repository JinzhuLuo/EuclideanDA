...
class Workspace:
    ...
    def get_eq_state(self, env):
        ...
        pos_arm = env.physics.named.data.xpos['arm'][None, :].flatten()
        pos_hand = env.physics.named.data.xpos['hand'][None, :].flatten()
        pos_finger = env.physics.named.data.xpos['finger'][None, :].flatten()


        vel_arm = env.physics.data.object_velocity("arm", "geom", local_frame=False)[0:1, :].flatten()
        vel_hand= env.physics.data.object_velocity("hand", "geom", local_frame=False)[0:1, :].flatten()
        vel_finger = env.physics.data.object_velocity("finger", "geom", local_frame=False)[0:1, :].flatten()


        eq_state = np.concatenate((pos_arm,pos_hand,pos_finger,vel_arm,vel_hand,vel_finger), axis=0)
        eq_state = eq_state.reshape(-1)

        to_target = env.physics.finger_to_target().flatten()
        eq_state = np.concatenate((eq_state, to_target), axis=0)
        eq_state = eq_state.reshape(-1)
        # print(eq_state.shape, to_target)
...
def main(cfg):
    from train_reacher import Workspace as W
    root_dir = Path.cwd()
    workspace = W(cfg)
