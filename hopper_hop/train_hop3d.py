...
class Workspace:
    ...
    def get_eq_state(self, env):
        ...
        pos_torso = env.physics.named.data.geom_xpos['torso_geom'][None, :].flatten()
        pos_pelvis = env.physics.named.data.geom_xpos['pelvis_geom'][None, :].flatten()
        pos_thigh = env.physics.named.data.geom_xpos['thigh_geom'][None, :].flatten()
        pos_calf = env.physics.named.data.geom_xpos['calf_geom'][None, :].flatten()
        pos_foot = env.physics.named.data.geom_xpos['foot_geom'][None, :].flatten()
        temp = pos_torso[0]
        pos_torso[0] -= temp
        pos_pelvis[0] -= temp
        pos_thigh[0] -= temp
        ...
        vel_torso = env.physics.data.object_velocity("torso_geom", "geom", local_frame=False)[0:1, :].flatten()
        vel_pelvis = env.physics.data.object_velocity("pelvis_geom", "geom", local_frame=False)[0:1, :].flatten()
        vel_thigh = env.physics.data.object_velocity("thigh_geom", "geom", local_frame=False)[0:1, :].flatten()
        vel_calf = env.physics.data.object_velocity("calf_geom", "geom", local_frame=False)[0:1, :].flatten()
        vel_foot = env.physics.data.object_velocity("foot_geom", "geom", local_frame=False)[0:1, :].flatten()

        eq_state = np.concatenate((pos_torso, pos_pelvis, pos_thigh, pos_calf, pos_foot, vel_torso, vel_pelvis, vel_thigh, vel_calf, vel_foot, pos_direction), axis=0)

        eq_state = eq_state.reshape(-1)
        # print(point_cloud_obs.shape)
        
        touch = env.physics.touch()
        eq_state = np.concatenate((eq_state, touch), axis=0)
        return eq_state
    
    
    
#        position = env.physics.data.qpos[1:].flatten()
#        velocity = env.physics.velocity().flatten()
#        touch = env.physics.touch().flatten()
#        
#        eq_state = np.concatenate((position, velocity, touch), axis=0)
#        eq_state = eq_state.reshape(-1)
#
#        return eq_state
...
def main(cfg):
    from train_hop3d import Workspace as W
    root_dir = Path.cwd()
    workspace = W(cfg)
